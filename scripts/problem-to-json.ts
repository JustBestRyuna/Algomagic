...
/**
 * 문제 MDC 파일을 JSON 형식으로 변환하는 스크립트
 * 
 * 사용법: ts-node scripts/problem-to-json.ts <MDC 파일 경로>
 * 예시: ts-node scripts/problem-to-json.ts ./problem.mdc
 */
...
if (args.length === 0) {
  ...
  console.error('사용법: ts-node scripts/problem-to-json.ts <MDC 파일 경로>');
  process.exit(1);
}
...
try {
  ...
  // JSON 파싱
  const problemData: ProblemData = JSON.parse(longestJsonMatch);
  ...
  if (!Array.isArray(problemData.examples) || problemData.examples.length === 0) {
    console.error('오류: examples는 최소 하나 이상의 항목을 포함해야 합니다.');
    process.exit(1);
  }
  
  // output 디렉토리 생성
  const outputDir = path.resolve(process.cwd(), 'data/problems', problemData.difficulty, problemData.category);
  fs.mkdirSync(outputDir, { recursive: true });
  
  // JSON 파일로 저장
  const outputPath = path.join(outputDir, `${problemData.id}.json`);
